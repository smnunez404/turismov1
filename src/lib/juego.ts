...
/** Tabla semanal ilustrativa: mezcla participantes locales con el jugador actual. */
export function tablaLiga(usuario: UsuarioSesion) {
  const otros = participantesRanking.slice(0, 14).map((p, i) => ({
    id: p.id,
    nombre: p.nombre,
    avatarId: p.avatarId,
    zona: p.zona,
    rachaDias: p.rachaDias,
    puntos: Math.max(20, Math.round(p.puntaje * 0.6) - i * 7),
    esUsuario: false,
  }));
  const equipo = equipos.find((item) => item.id === usuario.equipoId);
  const yo = {
    id: "yo",
    nombre: usuario.nombre || "Vos",
    avatarId: usuario.avatarId ?? "toborochi",
    zona: equipo?.zona ?? "Sin equipo",
